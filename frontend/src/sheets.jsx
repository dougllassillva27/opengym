...
import { EXDB, EXIDX, BODYPARTS, isCardio, allExercises, equipmentOf, exName } from './lib/exercises.js'
...
function ExercisePicker({ onPick, close }) {
  ...
  let base = all.filter(e =>
    ...
    (!ql || e.n.toLowerCase().includes(ql) || (e.nPt || '').toLowerCase().includes(ql) || (Array.isArray(e.aliases) ? e.aliases : []).some(a => a.toLowerCase().includes(ql)) || e.tg.includes(ql) || e.eq.includes(ql) || (e.desc || '').toLowerCase().includes(ql)))
